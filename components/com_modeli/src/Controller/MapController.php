...
namespace Viglin\Component\Modeli\Site\Controller;

\defined('_JEXEC') or die;

use Joomla\CMS\Factory;
use Joomla\CMS\MVC\Controller\BaseController;
use Viglin\Component\Modeli\Site\Helper\ModeliHelper;
...
class MapController extends BaseController
{
	public function pins()
	{
		try {
			...
			$model = $app->bootComponent('com_modeli')->getMVCFactory()->createModel('List', 'Site');
			$model->populateState();
			...
			foreach ($items as $item) {
				...
				$title = trim((string) ($item->title ?? $item->description ?? 'Поиск моделей'));
