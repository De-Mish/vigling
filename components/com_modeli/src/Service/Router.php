...
namespace Viglin\Component\Modeli\Site\Service;
...
class Router extends RouterBase
{
	...
	public function build(&$query): array
	{
		...
		if (
			$item
			&& isset($item->query['option'], $item->query['view'])
			&& $item->query['option'] === 'com_modeli'
			&& $item->query['view'] === 'list'
		) {
	...
	public function parse(&$segments): array
	{
		...
		if (!$item || !isset($item->query['option']) || $item->query['option'] !== 'com_modeli') {
			return $vars;
		}
