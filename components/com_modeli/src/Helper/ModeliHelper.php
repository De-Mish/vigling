...
namespace Viglin\Component\Modeli\Site\Helper;
...
class ModeliHelper
{
	private static ?array $searchMasterIdsCache = null;
	private static ?array $searchCategoryIdsCache = null;
	...
	public static function getSearchMasterIds(): array
	{
		if (is_array(self::$searchMasterIdsCache)) {
			return self::$searchMasterIdsCache;
		}
		...
		return self::$searchMasterIdsCache = array_values(array_filter(array_map('intval', (array) $rows)));
	}

	public static function getSearchCategoryIds(): array
	{
		if (is_array(self::$searchCategoryIdsCache)) {
			return self::$searchCategoryIdsCache;
		}
		...
		return self::$searchCategoryIdsCache = array_values(array_filter(array_map('intval', (array) $rows)));
	}

	public static function getCategories(): array
	{
		$categories = PoiskHelper::getCategories();
		$categoryIds = self::getSearchCategoryIds();
		if ($categoryIds === []) {
			return [];
		}
	...
	{
		$masterIds = self::getSearchMasterIds();
		if ($masterIds === []) {
			return [];
		}
