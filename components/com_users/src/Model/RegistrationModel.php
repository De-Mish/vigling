...
class RegistrationModel extends FormModel implements UserFactoryAwareInterface
{
    ...
    public function register($temp)
    {
        ...
        // MailTemplate performs string replacement; ensure no stdClass/array leaks into placeholders.
        $normalizeTemplateValue = static function ($value) use (&$normalizeTemplateValue) {
            ...
            if (\is_array($value)) {
                foreach ($value as $key => $item) {
                    $value[$key] = $normalizeTemplateValue($item);
                }
        ...
        foreach ($data as $key => $value) {
            $data[$key] = $normalizeTemplateValue($value);
        }
