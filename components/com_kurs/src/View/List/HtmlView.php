...
namespace Viglin\Component\Kurs\Site\View\List;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		/** @var \Viglin\Component\Kurs\Site\Model\ListModel $model */
		$model = $this->getModel();
		$model->populateState();
		...
		$this->categories = \Viglin\Component\Kurs\Site\Helper\KursHelper::getCategories();
		$this->cities = \Viglin\Component\Kurs\Site\Helper\KursHelper::getCities();
		$this->areas = \Viglin\Component\Kurs\Site\Helper\KursHelper::getAreas();
		$this->homeOptions = \Viglin\Component\Kurs\Site\Helper\KursHelper::getHomeOptions();
		...
		if ($catId && isset($this->categories[$catId])) {
			$this->pageTitle = 'Курсы: ' . trim((string) ($this->categories[$catId]['filter_title'] ?? $this->categories[$catId]['title'] ?? ''));
		}
		...
		if ($mapUserIds !== []) {
			$this->mapFieldsByUser = \Viglin\Component\Kurs\Site\Helper\KursHelper::getFieldsForUserIds($mapUserIds, [
