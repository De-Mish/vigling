...
$courseImage = trim((string) ($item->media_path ?? ''));
$courseImageUrl = $courseImage !== '' ? $resolveImageUrl($courseImage, true) : '';

if ($courseImageUrl !== '') {
	$cardImage = $courseImageUrl;
} else {
...
$courseTitle = trim((string) ($item->title ?? $item->description ?? ''));
$courseDescription = trim((string) ($item->description ?? ''));
	...
	<div class="category__item-content">
		<div class="category__item-content-left">
			<div class="category__content-info">
				<a class="category_cinfo-name" href="<?php echo $profileUrl; ?>"><?php echo htmlspecialchars($courseTitle !== '' ? $courseTitle : 'Курс', ENT_QUOTES, 'UTF-8'); ?></a>
				...
				<?php if ($courseDescription !== '') : ?>
					<span class="category_cinfo-spec"><?php echo htmlspecialchars(mb_substr($courseDescription, 0, 150), ENT_QUOTES, 'UTF-8') . (mb_strlen($courseDescription) > 150 ? '…' : ''); ?></span>
				<?php endif; ?>
				<?php if ($addr !== '') : ?>
				...
				<div class="category__content-info-list">
					<strong style="color:green;">Параметры курса</strong>
					<ul>
						...
						<?php if ($slotLabel !== '') : ?>
							<li><div>Дата и время: <span class="kurs-time-utc" data-time-utc="<?php echo htmlspecialchars($slotStartUtc, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($slotLabel, ENT_QUOTES, 'UTF-8'); ?></span></div></li>
						<?php endif; ?>
					</ul>
				</div>
				<?php if ($homeParts !== []) : ?>
