...
if (!empty($mapItems)) {
	foreach ($mapItems as $mapItem) {
		...
		$balloon = '<div class="map-balloon">'
			. '<a href="' . $profileLink . '">' . $courseTitle . '</a><br>'
			. '<span>Мастер: ' . $masterName . '</span><br>'
			. '<span>' . $addrEsc . '</span>'
			. '</div>';
		$mapMasters[] = [
			'course_id' => (int) ($mapItem->course_id ?? 0),
			'user_id' => (int) ($mapItem->master_id ?? 0),
			'name' => (string) ($mapItem->title ?? $mapItem->description ?? 'Курс'),
			'address' => $addr,
...
window.onKursYmapsReady = function() {
...
(function() {
	...
	function loadYmaps() {
		...
		script.onload = function() {
			if (typeof window.onKursYmapsReady === 'function') {
				window.onKursYmapsReady();
			}
		};
