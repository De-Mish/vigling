<?php
/**
 * Регистрирует плагин Ajax - Quickauth в #__extensions (быстрый вход/регистрация в модальном окне).
 * Запуск из корня сайта: php migration_scripts/30_register_plg_ajax_quickauth.php
 */
...
$manifestPath = $base . '/plugins/ajax/quickauth/quickauth.xml';
if (!is_file($manifestPath)) {
    die("Ошибка: не найден plugins/ajax/quickauth/quickauth.xml\n");
}

$xml = simplexml_load_file($manifestPath);
if (!$xml || $xml->getName() !== 'extension') {
    die("Ошибка: неверный manifest quickauth.xml\n");
}
...
if (strpos($name, 'PLG_') === 0) {
    $iniPath = $base . '/plugins/ajax/quickauth/language/ru-RU/plg_ajax_quickauth.sys.ini';
    if (is_file($iniPath)) {
...
$exists = $mysqli->query("SELECT extension_id FROM `{$table}` WHERE type = 'plugin' AND element = 'quickauth' AND folder = 'ajax' LIMIT 1");
...
if ($existing) {
    ...
    echo "Плагин Ajax - Quickauth уже в БД; имя и манифест (в т.ч. автор Vigling) обновлены.\n";
    $mysqli->close();
    exit(0);
}
...
$sql = "INSERT INTO `{$table}` (package_id, name, type, element, folder, client_id, enabled, access, protected, locked, manifest_cache, params, custom_data, ordering, state, changelogurl)
        VALUES (0, '{$name_esc}', 'plugin', 'quickauth', 'ajax', 0, 1, 1, 0, 0, '{$manifest_cache}', '{}', '', 0, 0, '')";
...
echo "Плагин Ajax - Quickauth зарегистрирован в БД (enabled=1).\n";
$mysqli->close();
