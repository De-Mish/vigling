<?php
/**
 * Добавляет фронтовый пункт меню «Поиск» по пути /modeli.
 *
 * Запуск после 66:
 *   php migration_scripts/67_add_modeli_menu.php
 */
...
$ext = $mysqli->query("SELECT extension_id FROM `{$prefix}extensions` WHERE type='component' AND element='com_modeli' LIMIT 1");
if (!$ext || !($e = $ext->fetch_object())) {
    die("Сначала выполните 66_register_com_modeli.php\n");
}
$componentId = (int) $e->extension_id;

$menuTable = $prefix . 'menu';
$has = $mysqli->query("SELECT id FROM `{$menuTable}` WHERE client_id=0 AND alias='modeli' LIMIT 1");
if ($has && $has->fetch_object()) {
    echo "Пункт меню modeli уже есть.\n";
    $mysqli->close();
    exit(0);
}
...
$rootId = 1;
$level = 1;
$title = $mysqli->real_escape_string('Поиск');
$alias = $mysqli->real_escape_string('modeli');
$path = $alias;
$link = $mysqli->real_escape_string('index.php?option=com_modeli&view=list');
...
if ($mysqli->query($query)) {
    echo "Пункт меню «Поиск» добавлен. URL: /modeli\n";
} else {
