<?php
/**
 * Мигратор legacy-акционных услуг (stock_prices) из #__fields_values в новую модель:
 * - #__vigling_user_stock_services
 * - #__vigling_service_unresolved
 *
 * Важно:
 * - в `#__vigling_user_stock_services` мигрируются только акционные услуги из поля `stock_prices`
 * - это отдельная таблица, чтобы акционные цены не перетирали обычные в `#__vigling_user_services`
 *
 * Опции:
 ...
 *
 * Запуск:
 *   php migration_scripts/47_vigling_stock_prices_migrator.php
 *   php migration_scripts/47_vigling_stock_prices_migrator.php --user-id=134438
 *   php migration_scripts/47_vigling_stock_prices_migrator.php --apply --truncate-target
 */
...
$tables = [
    ...
    'user_services' => $prefix . 'vigling_user_stock_services',
...
echo "=== Vigling Stock Prices Migrator ===\n";
echo "DB: {$config->db}\n";
