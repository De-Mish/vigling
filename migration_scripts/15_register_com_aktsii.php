<?php
/**
 * Регистрирует компонент com_aktsii в #__extensions (ручная установка).
 * Запуск из корня сайта: php migration_scripts/15_register_com_aktsii.php
 */
...
$manifestPath = $base . '/administrator/components/com_aktsii/aktsii.xml';
if (!is_file($manifestPath)) {
    die("Ошибка: не найден administrator/components/com_aktsii/aktsii.xml\n");
}

$xml = simplexml_load_file($manifestPath);
if (!$xml || $xml->getName() !== 'extension') {
    die("Ошибка: неверный manifest aktsii.xml\n");
}
...
$exists = $mysqli->query("SELECT extension_id FROM `{$table}` WHERE type = 'component' AND element = 'com_aktsii' LIMIT 1");
if ($exists && $exists->fetch_object()) {
    echo "Компонент com_aktsii уже зарегистрирован в БД.\n";
    $mysqli->close();
} else {
    ...
    $vals = "0, '{$name_esc}', 'component', 'com_aktsii', '', 1, 1, 1, 0, 0, '{$manifest_json}', '{}', '', 0, 0, ''";
    if (!$mysqli->query("INSERT INTO `{$table}` ({$cols}) VALUES ({$vals})")) {
        die("Ошибка INSERT: " . $mysqli->error . "\n");
    }
    echo "Компонент com_aktsii зарегистрирован в БД (enabled=1).\n";
    $mysqli->close();
}
...
if (is_file($autoloadFile)) {
    ...
    if (strpos($content, "Viglin\\\\Component\\\\Aktsii\\\\") === false) {
        $insert = "\t'Viglin\\\\Component\\\\Aktsii\\\\Administrator\\\\' => [JPATH_ADMINISTRATOR . '/components/com_aktsii/src'],\n\t'Viglin\\\\Component\\\\Aktsii\\\\Site\\\\' => [JPATH_SITE . '/components/com_aktsii/src'],";
        $content = str_replace(
        ...
        if (file_put_contents($autoloadFile, $content) !== false) {
            echo "В autoload_psr4.php добавлен namespace для com_aktsii.\n";
        }
    }
}

echo "Создайте пункт меню: тип com_aktsii → Список. Алиас: poisk-aktsij.\n";
echo "Тогда ссылки /poisk-aktsij и /poisk-aktsij/16 будут открывать страницу «Поиск акций».\n";
