...
final class Joomla extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onUserLogin(LoginEvent $event)
    {
        ...
        if (is_file($servicePath)) {
            ...
            if (class_exists($serviceClass)) {
                try {
                    ...
                    if ($serviceClass::enforceGraceForUser($db, (int) $instance->id)) {
                        $instance->block = 1;
                    }
                } catch (\Throwable $e) {
                    // Ignore verification check failures to avoid login hard-fail when DB/plugin is unavailable.
                }
            }
        }
        ...
        if ($instance->block == 1) {
            $messageKey = 'JERROR_NOLOGIN_BLOCKED';
            $showResend = false;
            $resendEmail = '';
            $resendReason = 'blocked';
            if (isset($serviceClass) && class_exists($serviceClass)) {
                try {
                    $db = $this->getDatabase();
                    if ($serviceClass::isBlockedByUnverified($db, (string) $instance->username)) {
                        $messageKey = 'PLG_SYSTEM_EMAILVERIFICATION_ACCOUNT_BLOCKED_WARNING';
                        $showResend = true;
                        $resendEmail = (string) ($instance->email ?? '');
                        $resendReason = 'email_verification_blocked';
                        $this->getApplication()->getLanguage()->load('plg_system_emailverification', JPATH_SITE . '/plugins/system/emailverification');
                    }
                } catch (\Throwable $e) {
                    // Fallback to default blocked message.
                }
            }
            $this->getApplication()->setUserState('vigling.emailverification.show_resend', $showResend ? 1 : 0);
            $this->getApplication()->setUserState('vigling.emailverification.resend_email', $resendEmail);
            $this->getApplication()->setUserState('vigling.emailverification.reason', $resendReason);
            $this->getApplication()->setUserState('vigling.emailverification.message', $this->getApplication()->getLanguage()->_($messageKey));
            $this->getApplication()->enqueueMessage($this->getApplication()->getLanguage()->_($messageKey), 'warning');
            $event->addResult(false);
